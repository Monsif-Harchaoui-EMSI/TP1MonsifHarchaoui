...
public class JsonUtilPourGemini implements Serializable {
    ...
    private String systemRole= "Repond moi comme un Psychiatre calmant";
    /**
    ...
    private String creerRequeteJson(String systemRole, String question) {
        if (systemRole == null || question == null) {
            throw new IllegalArgumentException("System role and question cannot be null");
        }
    ...
    private String extractReponse(String json) throws RequeteException {
        try (JsonReader jsonReader = Json.createReader(new StringReader(json))) {
            ...
            if (!jsonObject.containsKey("candidates")) {
                throw new RequeteException("La réponse ne contient pas le champ 'candidates'", json);
            }

            JsonArray candidates = jsonObject.getJsonArray("candidates");
            if (candidates.isEmpty()) {
                throw new RequeteException("Le tableau 'candidates' est vide", json);
            }

            JsonObject messageReponse = candidates.getJsonObject(0).getJsonObject("content");
